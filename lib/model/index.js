...
utils.mixin(model, new (function () {
  ...
  this._addError = function (item, params) {
    geddy.log.debug(JSON.stringify(params));

    var error, locale = geddy.config.i18n.defaultLocale;
    error = 'model.validates' + utils.string.capitalize(
      params.err.validator || params.validator);
    ...
    if (typeof params.err == 'string') {
      item.errors[params.name] = params.err;
    }
    else {
      item.errors[params.name] = i18n.getText(error, params.err, locale); 
    }
  };
