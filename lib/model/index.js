...
utils.mixin(model, new (function () {
  ...
  this.addError = function (item, params) {
    var error, locale = geddy.config.i18n.defaultLocale;
    error = 'model.validates' + geddy.string.capitalize(params.validator);

    if (!item.errors) {
      item.errors = {};
    }

    item.errors[field] = i18n.getText(error, data, locale); 
  };
